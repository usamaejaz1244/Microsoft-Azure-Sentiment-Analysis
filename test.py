...
'''
# create an empty dataframe to store the desired attributes
yelp_review_1000 = pd.DataFrame(columns=['', 'review_id', 'user_id', 'business_id' 'stars', 'useful', 'funny', 'cool', 'text', 'date'])

# define the chunk size
chunk_size = 10000

# read the csv file in chunks
for chunk in pd.read_csv('yelp_review.csv', chunksize=chunk_size):
    # extract the desired attributes from the chunk
    chunk_attributes = chunk[['', 'review_id', 'user_id', 'business_id' 'stars', 'useful', 'funny', 'cool', 'text', 'date']]

    # append the extracted attributes to the new_df dataframe
    yelp_review_1000 = yelp_review_1000.append(chunk_attributes, ignore_index=True)
